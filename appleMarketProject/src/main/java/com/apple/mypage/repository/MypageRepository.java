...
public interface MypageRepository extends JpaRepository<Test_order, String> {
    ...
    // 판매완료 상품
    @Query("SELECT i FROM Product i WHERE i.user.userNo = :userNo AND i.productID IN (SELECT o.item.productID FROM Test_order o)")
    ...
    @Transactional
    @Modifying
    @Query("DELETE FROM Test_order o WHERE o.item.productID = :productID AND o.user.userNo = :userNo")
    void deleteOrderByUserNoAndProductId(@Param("userNo") Long userNo, @Param("productID") Long productID);
    
    // 판매 상태를 가져오는 쿼리
    @Query("SELECT CASE WHEN EXISTS (SELECT o FROM Test_order o WHERE o.item.productID = i.productID) THEN 'sold' ELSE 'selling' END " +

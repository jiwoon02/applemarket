...
public interface MypageRepository extends JpaRepository<Order, String> {
    ...
    @Query("SELECT o.product FROM Order o WHERE o.user.userNo = :userNo AND o.orderRegDate >= :threeMonthsAgo")
    ...
    // 판매 상태를 가져오는 쿼리
    @Query("SELECT CASE WHEN EXISTS (SELECT o FROM Order o WHERE o.product.productID = i.productID) THEN 'sold' ELSE 'selling' END " +
